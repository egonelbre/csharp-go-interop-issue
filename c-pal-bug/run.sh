#!/usr/bin/env bash
# Build and run the .NET + C sigaltstack overflow reproducer.
# This demonstrates that CoreCLR's PAL bug is not Go-specific.
#
# Usage:
...
#   ./run.sh fix              Run with workaround enabled.
#   ./run.sh gdb              Build, then run under gdb and dump a core at crash.
#   ./run.sh loop [N]         Run N times in a row (default 10) and report rc.
#
# Requirements: .NET SDK (10.0+), gcc, gdb (for `gdb` mode).
set -euo pipefail

cd "$(dirname "$0")"

DOTNET_BIN="./bin/Release/net10.0/repro-c-pal"

build() {
    echo "=== building libclib.so (C library) ==="
    make clean && make all
